...
| `RECURSION_COMPARISON_NOTES.md` | **Answer-sheet** for the recurring flat-vs-recursion questions: the segment/outer/aggregate decomposition, per-metric aggregation rules, deployment models (parallel/sequential/concurrent + the Amdahl gap), the comparison taxonomy, verifier-time noise, secondary metrics, scaling-beyond-1000, and the session's decisions. |
...
| `RELATED_WORK.md` | Comparison-chapter scaffold: SOTA grouped by frontier corner (distributed/collaborative proving, recursion/folding/aggregation/commit-and-prove, zkVM continuations, ZK-for-graphs/routing), each with a 2-sentence summary + relevance, a synthesis table, and the gap. **Comparison vs current research — not background.** |
