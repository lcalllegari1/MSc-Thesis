...
K=2), so the glue is not separately measured. The whole **outer is the binding layer**
— the recursion analogue of the hierarchical glue + verifier tax. So the recursion
split is *segments vs outer*, and the §E stacked bar reads "cheap parallel leaf
proving + one expensive monolithic aggregation," not "segments + glue."
...
**Data source.** Run the aggregators with `--split-components` (both
`aggregate_hier.py` and `aggregate_recursion.py`). Each cell then emits, beside the
combined row, separate per-component rows — `{variant}_k{K}_seg` + `_glue` for the
hierarchical family, `recursion_k{K}_seg` + `_outer` for recursion (recursion has
**no `_glue`**: the glue is fused into the outer with the K in-circuit verifications,
so the binding component there is the whole outer — see §2.1). The components sum
back to the combined row, so they are a faithful decomposition. `plot.py` reads them
as ordinary variants/lines (`--variants '*_seg' '*_glue'`); the stacked-bar rendering
...
- **E1. Stacked bar of `circuit_size` = sub-share vs glue-share** (vs commitment-fold
  ...
  outer's ~96%-verification / ~4%-glue makeup is itself the story.)
