"""
pipeline/aggregate_composite.py -- Convert raw hierarchical-benchmark CSV (K+1 rows
per cell) into a plot.py-compatible CSV (one row per (N, K, run)).

run_composite_plain_sort.py (Variant A) and run_composite_plain_product.py (Variant A++) both emit K+1 rows per
(N, K, run) cell -- one row per circuit: sub_0, sub_1, ..., sub_{K-1}, glue.
plot.py expects one row per (variant, N) data point with the standard run_monolithic.py
...
column, so the SAME script handles every hierarchical variant with no flags:
  results/composite_plain_sort.csv    -> rows tagged  composite_plain_sort_k{K}     (Variant A)
  results/composite_plain_product.csv   -> rows tagged  composite_plain_product_k{K}    (Variant A++)
  results/composite_committed_sort.csv    -> rows tagged  composite_committed_sort_k{K}     (Variant committed-A)
  results/composite_committed_product.csv  -> rows tagged  composite_committed_product_k{K}   (Variant committed-A++)
...
-----------------
  ...
  verify_s       | sum + xcheck  | bb verify runs serially K+1 times; add the
                 |               |  off-circuit verify_composite_s for cross-checks
...
CSV's `variant` column (`composite_plain_sort` or `composite_plain_product`) -- one variant per K value,
...
lines (never collapsed into one max):
  ...
  {base}_k{K}_glue       the GLUE proof alone (its own measured values).  The
                         external cross-check time (verify_composite_s) rides with
...
Matches pipeline/run_monolithic.py exactly:
...
-----
  ...
  python pipeline/aggregate_composite.py \\
      --in  results/composite_plain_sort.csv \\
      --out results/composite_plain_sort_parallel.csv \\
      --mode parallel

  # Same data, total CPU work.
  python pipeline/aggregate_composite.py \\
      --in  results/composite_plain_sort.csv \\
      --out results/composite_plain_sort_total.csv \\
      --mode total

  # Plot side-by-side with the existing flat baseline.
  python pipeline/plot.py \\
      --csv results/500.csv results/composite_plain_sort_parallel.csv \\
...
def aggregate(rows, mode, include_mode_in_name, split_components=False):
    """
    Group raw composite_plain_sort rows by (n, k, run); emit one aggregated dict per group.
    """
    ...
    for (n, k, run), group in sorted(grouped.items()):
        ...
        verify_composite_s = float(subs[0].get("verify_composite_s", "0") or "0")
        verify_s += verify_composite_s
        ...
        # aggregator serves Variant A (composite_plain_sort) and A++ (composite_plain_product) unchanged:
        # composite_plain_sort -> composite_plain_sort_k{K}, composite_plain_product -> composite_plain_product_k{K}.
        base = subs[0].get("variant", "composite_plain_sort") or "composite_plain_sort"
        ...
        # verify_composite_s cross-check rides with it (the verifier-side binding tax).
        if split_components:
            ...
            out.append(_mkrow(
                ...
                float(glue["verify_s"]) + verify_composite_s,
...
def main():
    ap = argparse.ArgumentParser(
        description="Aggregate raw composite_plain_sort CSV into a plot.py-compatible CSV."
    )
    ap.add_argument("--in", dest="inp", required=True, type=Path,
                    help="Input CSV from pipeline/run_composite_plain_sort.py")
    ...
    ap.add_argument("--mode-in-name", action="store_true",
                    help="Append _<mode> to the variant column "
                         "(e.g. composite_plain_sort_k4_parallel) so both modes can be "
