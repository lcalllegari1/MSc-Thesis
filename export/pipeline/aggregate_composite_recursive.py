"""
pipeline/aggregate_composite_recursive.py -- Convert raw recursion micro-experiment CSV
(pipeline/run_composite_recursive.py output) into a plot.py-compatible CSV
(one row per (exp, N, run)), so recursion sits on the same axes as flat_* and
the aggregated hier_* variants.

This is the recursion analogue of pipeline/aggregate_composite.py.  The raw CSV has
...
row per (variant, N) data point in the run_monolithic.py schema, so we fold each
...
Compared with aggregate_composite.py this makes recursion's profile explicit: high
...
aggregate_composite.py uses (so recursion and A++ compare apples-to-apples).  This is
...
--------------
  ...
  exp 2 -> "{label}_k{K}"   (the complete recursive proof of the K-cycle; the
                             row to overlay with monolithic_committed / composite_plain_product_k{K})
...
Schema written (matches pipeline/run_monolithic.py exactly)
------------------------------------------------
...
Usage
-----
  python pipeline/aggregate_composite_recursive.py \\
  ...
  python pipeline/plot.py \\
      --csv results/500.csv results/composite_plain_product_par.csv results/recursion_par.csv \\
...
def main():
    ...
    ap.add_argument("--in", dest="inp", required=True, type=Path,
                    help="Input CSV from pipeline/run_composite_recursive.py")
    ...
    ap.add_argument("--label", default="composite_recursive",
