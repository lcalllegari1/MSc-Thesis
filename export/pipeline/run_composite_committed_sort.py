"""
pipeline/run_composite_committed_sort.py -- Variant committed-A hierarchical TSP benchmark harness.

Structurally identical to pipeline/run_composite_plain_sort.py (Variant A); the differences are
mechanical:
  * Circuits: composite_committed_sort_segment / composite_committed_sort_glue (Variant committed-A:
              the segment node set + cost are hidden behind a blinded Poseidon2
              commitment C_i instead of the public sorted_nodes; the partition is
              the same O(N) sort, now over the witnessed, commitment-bound nodes).
  * Builder:  merkle_builder --composite-committed-sort K  (folds each segment + a uniform
              blinding r into C_i; the glue recomputes and binds them).
  * Verifier: pipeline/verify_composite_committed_sort.py (cross-checks the shared root and that
              glue.C_is[i] == sub_i.C_i; no Fiat-Shamir, no per-field bookkeeping).
  * Shadows:  /tmp/composite_committed_sort_shadows (does not collide with A's /tmp/composite_plain_sort_shadows).
  * CSV variant column: "composite_committed_sort".

The CSV schema is identical to run_composite_plain_sort.py's (K+1 rows per cell), so
pipeline/aggregate_composite.py consumes results/composite_committed_sort.csv with no changes and emits
composite_committed_sort_k{K} variant rows for plot.py.

For each (N, K) the script:
  1. Patches N, M, DEPTH (sub) and N, K, M, DEPTH (glue) in both .nr files and
     recompiles + writes VKs.
  2. Sets up K shadow dirs for the sub-circuit (self-contained compiled copies).
  3. For each run: generates an instance, solves a cycle, builds the K+1
     Prover.tomls via merkle_builder --composite-committed-sort, then launches K+1
     parallel `nargo execute && bb prove` workers.
  4. Runs verify_composite_committed_sort.py to sanity-check the cross-checks.

Output CSV columns (one row per circuit, K+1 rows per cell):
    variant n k m run circuit circuit_size acir_opcodes compile_s
    witness_s prove_s verify_s proof_bytes peak_mb verify_composite_s xchecks_ok

Usage:
    python pipeline/run_composite_committed_sort.py \\
        --ns 48 96 192 480 \\
        --ks 2 4 8 \\
        --runs 3 \\
        --out results/composite_committed_sort.csv
"""
...
SUB_DIR      = PROJECT_ROOT / "circuits" / "composite_committed_sort_segment"
GLUE_DIR     = PROJECT_ROOT / "circuits" / "composite_committed_sort_glue"
SUB_NAME     = "composite_committed_sort_segment"
GLUE_NAME    = "composite_committed_sort_glue"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
VERIFY_HIER_C = PROJECT_ROOT / "pipeline" / "verify_composite_committed_sort.py"

# Per-N-K sub-circuit shadow dirs, hosted under /tmp/ (see run_composite_plain_sort.py rationale).
# Distinct from A's /tmp/composite_plain_sort_shadows so both harnesses can run concurrently.
SHADOW_ROOT  = Path("/tmp/composite_committed_sort_shadows")
...
# ── CSV layout ────────────────────────────────────────────────────────────────
FIELDNAMES = [
    "variant",       # always "composite_committed_sort"
    "n", "k", "m",   # geometry
    ...
    "verify_composite_s", # wall time for pipeline/verify_composite_committed_sort.py (same value all K+1 rows)
    "xchecks_ok",    # 1 if verify_composite_committed_sort accepted, else 0
]
...
def configure_circuits(n, k):
    ...
    patch_globals(GLUE_DIR / "src" / "main.nr", N=n, K=k, M=m, DEPTH=depth)
...
def make_shadow_dirs(k):
    """
    Create K self-contained compiled copies of the sub-circuit so K parallel
    nargo execute / bb prove invocations don't race on shared target/<name>.gz.
    (Same rationale as run_composite_plain_sort.py.)
    """
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --composite-committed-sort."""
...
def build_hier_tomls(payload_dict, k, out_dir, tree_cache=None):
    """Invoke merkle_builder --composite-committed-sort K --out-dir <dir>."""
    if out_dir.exists():
        shutil.rmtree(out_dir)
    payload = json.dumps(payload_dict)
    cmd = [str(BUILDER_BIN), "--composite-committed-sort", str(k), "--out-dir", str(out_dir)]
...
def run_verify_composite_committed_sort(scratch, n, k):
    """Run pipeline/verify_composite_committed_sort.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
        "python3", str(VERIFY_HIER_C),
...
def benchmark(ns, ks, runs, out_csv, seed, isolated=False):
    ...
    variant_tag = "composite_committed_sort" + ("_iso" if isolated else "")
        ...
        for n in ns:
            ...
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    cell_dir  = Path(f"/tmp/run_composite_committed_sort/n{n}_k{k}/run{run_idx}")
                    if cell_dir.exists():
                    ...
                    vh_rc, vh_s = run_verify_composite_committed_sort(scratch, n, k)
                    ...
                    for r in res:
                        ...
                        writer.writerow({
                            ...
                            "verify_composite_s":round(vh_s, 4),
                    ...
                    print(
                        ...
                        f"verify_composite_committed_sort={'OK' if xcheck_ok else 'FAIL'}({vh_s:.2f}s)"
                    )
...
def main():
    ap = argparse.ArgumentParser(description="Variant committed-A hierarchical TSP benchmark sweep.")
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/composite_committed_sort.csv")
