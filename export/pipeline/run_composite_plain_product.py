"""
pipeline/run_composite_plain_product.py -- Variant A++ hierarchical TSP benchmark harness.
...
mechanical (HIER_FS_IMPL.md sec. 6):
  * Circuits: composite_plain_product_segment / composite_plain_product_glue.
  * Builder:  merkle_builder --composite-plain-product K  (adds the FS chain, the
              per-segment grand products P_i, and the chain anchors).
  * Verifier: pipeline/verify_composite_plain_product.py (cross-checks root/c/X and the
              per-segment P_i / h_in / h_out, no field arithmetic).
  * Shadows:  /tmp/composite_plain_product_shadows (does not collide with A's /tmp/composite_plain_sort_shadows).
  * CSV variant column: "composite_plain_product".

The CSV schema is identical to run_composite_plain_sort.py's (K+1 rows per cell), so
pipeline/aggregate_composite.py consumes results/composite_plain_product.csv with no changes and emits
composite_plain_product_k{K} variant rows for plot.py.
...
For each (N, K) the script:
  ...
  3. For each run: generates an instance, solves a cycle, builds the K+1
     Prover.tomls via merkle_builder --composite-plain-product, then launches K+1
     parallel `nargo execute && bb prove` workers.
  4. Runs verify_composite_plain_product.py to sanity-check the cross-checks.
...
Usage:
    python pipeline/run_composite_plain_product.py \\
        --ns 48 96 192 480 \\
        --ks 2 4 8 \\
        --runs 3 \\
...
SHADOW_ROOT  = Path("/tmp/composite_plain_product_shadows")
...
# ── CSV layout ────────────────────────────────────────────────────────────────
FIELDNAMES = [
    "variant",       # always "composite_plain_product"
    "n", "k", "m",   # geometry
    ...
    "verify_composite_s", # wall time for pipeline/verify_composite_plain_product.py (same value all K+1 rows)
    "xchecks_ok",    # 1 if verify_composite_plain_product accepted, else 0
]
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --composite-plain-product."""
...
def build_hier_tomls(payload_dict, k, out_dir, tree_cache=None):
    """Invoke merkle_builder --composite-plain-product K --out-dir <dir>."""
    if out_dir.exists():
        shutil.rmtree(out_dir)
    payload = json.dumps(payload_dict)
    cmd = [str(BUILDER_BIN), "--composite-plain-product", str(k), "--out-dir", str(out_dir)]
...
def run_verify_composite_plain_product(scratch, n, k):
    """Run pipeline/verify_composite_plain_product.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
...
def benchmark(ns, ks, runs, out_csv, seed, isolated=False):
    ...
    variant_tag = "composite_plain_product" + ("_iso" if isolated else "")
        ...
        for n in ns:
            ...
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    cell_dir  = Path(f"/tmp/run_composite_plain_product/n{n}_k{k}/run{run_idx}")
                    if cell_dir.exists():
                    ...
                    vh_rc, vh_s = run_verify_composite_plain_product(scratch, n, k)
                    ...
                    print(
                        ...
                        f"verify_composite_plain_product={'OK' if xcheck_ok else 'FAIL'}({vh_s:.2f}s)"
                    )
...
def main():
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/composite_plain_product.csv")
