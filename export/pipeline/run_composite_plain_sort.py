"""
pipeline/run_composite_plain_sort.py — Variant A hierarchical TSP benchmark harness.

Mirrors pipeline/run_monolithic.py for the flat circuits.  Differences:
  * Two compiled circuits per (N, K) cell: composite_plain_sort_segment and
    composite_plain_sort_glue.  Patcher handles N + M = N/K + K + DEPTH together.
  ...
  * K+1 CSV rows per (N, K, run), distinguished by a `circuit` column with
    ...
    total CPU work.
  * The verifier cross-check (pipeline/verify_composite_plain_sort.py) runs once per cell to
...
For each (N, K) the script:
  ...
  3. For each run, generates an instance, solves a cycle, builds the K+1
     Prover.tomls via the Rust merkle_builder --composite-plain-sort, then launches
  ...
  4. Runs verify_composite_plain_sort.py against the produced proofs to sanity-check the
...
Usage:
    python pipeline/run_composite_plain_sort.py \\
        --ns 48 96 192 480 \\
        --ks 2 4 8 \\
        --runs 3 \\
        --out results/composite_plain_sort.csv
"""
...
SUB_DIR      = PROJECT_ROOT / "circuits" / "composite_plain_sort_segment"
GLUE_DIR     = PROJECT_ROOT / "circuits" / "composite_plain_sort_glue"
SUB_NAME     = "composite_plain_sort_segment"
GLUE_NAME    = "composite_plain_sort_glue"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
VERIFY_HIER  = PROJECT_ROOT / "pipeline" / "verify_composite_plain_sort.py"

# Per-N-K sub-circuit shadow dirs.  Hosted under /tmp/ rather than
# inside circuits/composite_plain_sort_segment/.runs/ because Nargo walks up looking
...
SHADOW_ROOT  = Path("/tmp/composite_plain_sort_shadows")
...
# ── CSV layout ────────────────────────────────────────────────────────────────
FIELDNAMES = [
    "variant",       # always "composite_plain_sort"
    "n", "k", "m",   # geometry
    ...
    "verify_composite_s", # wall time for pipeline/verify_composite_plain_sort.py (same value all K+1 rows)
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --composite-plain-sort."""
...
def build_hier_tomls(payload_dict, k, out_dir, tree_cache=None):
    """Invoke merkle_builder --composite-plain-sort K --out-dir <dir>."""
    if out_dir.exists():
        shutil.rmtree(out_dir)
    payload = json.dumps(payload_dict)
    cmd = [str(BUILDER_BIN), "--composite-plain-sort", str(k), "--out-dir", str(out_dir)]
...
def run_verify_hier(scratch, n, k):
    """Run pipeline/verify_composite_plain_sort.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
...
def benchmark(ns, ks, runs, out_csv, seed, isolated=False):
    ...
    variant_tag = "composite_plain_sort" + ("_iso" if isolated else "")
        ...
        for n in ns:
            ...
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    cell_dir  = Path(f"/tmp/run_composite_plain_sort/n{n}_k{k}/run{run_idx}")
                    if cell_dir.exists():
...
def main():
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/composite_plain_sort.csv")
