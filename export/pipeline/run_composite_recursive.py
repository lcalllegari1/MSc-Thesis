"""
pipeline/run_composite_recursive.py -- recursion variant benchmark harness.

Production mode:

  --exp 2   Recursively verify ALL K segments of a K-way split AND fold in the
            full glue logic (outer circuit = circuits/composite_recursive, generic over K).
            This is the shipped recursive proof: one outer proof exposing only
            (root, threshold).

The old --exp 1 single-segment diagnostic mode belongs to the experimental
workspace and is intentionally not shipped in this export.

The harness reuses the UNMODIFIED composite_plain_product_segment circuit as the inner (the *plain*
...
the inners (it consumes their proofs), a SERIAL tail.  aggregate_composite_recursive.py then
...
What it does per (N, K, run):
  1. Patch + compile composite_plain_product_segment for (N, M=N/K, DEPTH); write its
  ...
  2. Generate a TSP instance, solve it, and build the K+1 Prover.tomls via
     merkle_builder --composite-plain-product K.
...
The composite_plain_product_segment source globals are snapshotted and RESTORED at the
...
(Feed this raw CSV to pipeline/aggregate_composite_recursive.py to get plot.py-compatible
...
Smoke usage (small; validates the harness):
    /home/callexyz/anaconda3/envs/zk-tsp/bin/python \\
        pipeline/run_composite_recursive.py --exp 2 --n 8 --k 2 --runs 1 \\
...
# exp 2 is THE recursion variant; its outer circuit lives in circuits/composite_recursive.
EXP2_DIR     = PROJECT_ROOT / "circuits" / "composite_recursive"
EXP2_NAME    = "composite_recursive"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
SCRATCH_ROOT = Path("/tmp/composite_recursive_run")
...
def build_tomls(n, k, instance, cycle, out_dir, tree_cache=None, multiplier=1.1):
    """merkle_builder --composite-plain-product K -> out_dir/{sub_0..,glue}/Prover.toml."""
    if out_dir.exists():
...
def assemble_exp2(segs, vk_fields, key_hash, tomls_dir):
    ...
    lines = [
        "# Auto-generated by run_composite_recursive.py (exp 2) -- do not edit by hand\n",
...
def main():
    ...
    ap.add_argument("--exp", type=int, choices=[2], default=2,
                    help="Production recursion variant. Only exp=2 is shipped in this export.")
    ...
    outer_dir, outer_name = EXP2_DIR, EXP2_NAME
