"""
benchmark/run_monolithic.py  --  Run the ZKP pipeline across a range of N values and record metrics.
...
Usage:
    python benchmark/run_monolithic.py \\
        --circuit circuits/monolithic_study_pairwise \\
        --ns 5 8 10 15 20 \\
        --runs 3 \\
        --out results/monolithic_study_pairwise.csv
"""
...
# Path to the compiled Rust Merkle builder (used for monolithic_study_committed_presence).
...
# ── CSV column order ──────────────────────────────────────────────────────────
FIELDNAMES = [
    "variant",       # circuit directory name, e.g. monolithic_study_pairwise
    "n",             # number of nodes
...
def set_circuit_n(circuit_dir, n):
    ...
    Also patches (if present):  global DEPTH: u32 = <depth>;
      DEPTH is used by monolithic_study_committed_presence to set the Merkle tree depth.
...
def main():
    ...
    parser.add_argument(
        ...
        help="Path to the circuit directory (e.g. circuits/monolithic_study_pairwise)",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--out", required=True,
        help="Output CSV path, e.g. results/monolithic_study_pairwise.csv",
    )
    parser.add_argument(
