"""
pipeline/verify_composite_committed_sort.py — Variant committed-A hierarchical verifier.
...
Difference from Variant A's verify_composite_plain_sort.py: the per-segment values
...
Usage:
    python pipeline/verify_composite_committed_sort.py --proof-dir <dir> --n 8 --k 2 \
        --sub-vk  circuits/composite_committed_sort_segment/target/vk/vk \
        --glue-vk circuits/composite_committed_sort_glue/target/vk/vk
"""
...
def verify_composite_committed_sort(proof_dir: Path, n: int, k: int, sub_vk: Path, glue_vk: Path) -> int:
    ...
    print(f"verify_composite_committed_sort: N={n} K={k} M={m}")
...
def main():
    ...
    sys.exit(verify_composite_committed_sort(args.proof_dir, args.n, args.k, args.sub_vk, args.glue_vk))
