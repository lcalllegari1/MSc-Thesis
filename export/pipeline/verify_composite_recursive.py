"""
pipeline/verify_composite_recursive.py — Recursion variant verifier.

The recursion variant collapses the K+1 independent proofs of the hierarchical
family into ONE outer proof (circuits/composite_recursive).  The outer circuit recursively
verifies the K composite_plain_product_segment (A++) proofs in-circuit and re-runs the
glue logic on their now-trusted-but-private public inputs, so the entire binding
that verify_composite_plain_product.py does externally is folded inside the proof.

Therefore this verifier is the PHOTO-NEGATIVE of verify_composite_plain_product.py:

  * verify_composite_plain_product.py: K+1 `bb verify` calls + ~6K+3 Field-equality cross-checks
    (the verifier-side binding tax).
  * verify_composite_recursive.py: exactly ONE `bb verify`, ZERO cross-checks.  The binding
    moved into the circuit, so there is nothing left to cross-check externally.
    Public surface is just (root, threshold) — same as monolithic_committed.
...
Usage:
    python pipeline/verify_composite_recursive.py --proof-dir <dir> \
        --vk circuits/composite_recursive/target/vk/vk
"""
