"""
tests/correctness/test_composite_committed_sort.py

Soundness tests for Variant committed-A (hierarchical TSP, Merkle commitment,
partition HIDDEN behind a blinded Poseidon2 commitment C_i; sort-based partition
check in the glue over the now-witnessed nodes).
Mirrors test_composite_plain_sort.py; adapts the negative tests to the committed public
surface (sorted_nodes / endpoints / partial_cost are no longer public, so
per-field tampering surfaces at the commitment check, and the partition check is
the glue's O(N) sort over the witnessed nodes).

committed-A produces K+1 independent UltraHonk proofs (K sub-proofs of
circuits/composite_committed_sort_segment + 1 glue proof of composite_committed_sort_glue), bound by
verifier-side cross-checks in pipeline/verify_composite_committed_sort.py (same root across K+1, and
glue.C_is[i] == sub_i.C_i).  No Fiat-Shamir; the binding is purely the commitment.
...
Test list:
  ...
  5. partition_overlap (inh.) -- node 3 in both segments; glue G2 sort rejects
                                 (sorted(all_nodes) != [0..N-1]).
  6. cross_check (NEW)        -- mix sub-proofs from cycle A with glue from cycle B;
                                 verify_composite_committed_sort rejects (C_i mismatch).
  7. sanity_N48_K4            -- witness-only at a larger size.

Usage:
    python tests/correctness/test_composite_committed_sort.py
...
VERIFY_HIER    = PROJECT_ROOT / "pipeline" / "verify_composite_committed_sort.py"
BUILDER_FLAG   = "--composite-committed-sort"

WORK_DIR       = Path("/tmp/test_composite_committed_sort")
...
def configure_circuits(n, k):
    """Patch and recompile both circuits for (N, K).  M = N/K.  The committed-A
    glue needs M (it slices all_nodes into per-segment chunks)."""
    if n % k != 0:
    ...
    _patch_globals(GLUE_DIR / "src" / "main.nr", N=n, K=k, M=m, DEPTH=depth)
    compile_circuits()
...
def assert_baseline_passes():
    ...
    if rc != 0:
        print(f"  [FAIL] VALID baseline: verify_composite_committed_sort rejected:\n{out.strip()}"); return False
...
def assert_partition_overlap_rejected():
    """Glue G2 sort: node 3 in both segments, 7 dropped; sorted != [0..N-1]."""
    n, k = 8, 2
    flat = [0] * (n * n)
    cheat_edges = [
        (0, 5, 10), (5, 3, 12), (3, 2,  8),
    ...
    rc, err = run_glue_witness(hier_dir)
    if rc == 0:
        print("  [FAIL] INVALID overlap: glue accepted overlapping partition (BUG: G2 hole)"); return False
    if "partition" not in err and "cover" not in err and "Failed" not in err:
        print(f"  [WARN] INVALID overlap: rejected but error doesn't mention partition:\n{err.strip()}")
    print("  [OK  ] INVALID overlap: glue G2 sort rejected the overlapping partition")
    return True
...
def assert_cross_check_rejected():
    ...
    DIFFERENT cycles.  Mix sub_0+sub_1 from set A with glue from set B.  Each bb
    verify accepts; verify_composite_committed_sort rejects because glue.C_is[i] != sub_i.C_i.
    """
    n, k = 8, 2
    flat = [0] * (n * n)
    edges = [
    ...
    if rc == 0:
        print("  [FAIL] INVALID cross-check: verify_composite_committed_sort accepted mixed proofs (BUG)")
    ...
    print("  [OK  ] INVALID cross-check: verify_composite_committed_sort rejected mixed-cycle proof set")
    return True
...
def main():
    ...
    print("\n--- Negative test: partition overlap (sort) ---")
    results.append(assert_partition_overlap_rejected())

    print("\n--- Negative test: verifier cross-check (commitment binding) ---")
