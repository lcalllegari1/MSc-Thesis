"""
tests/correctness/test_composite_plain_sort.py

Soundness tests for Variant A (hierarchical TSP, Merkle commitment).
Mirrors the structure of test_monolithic_study_committed_presence.py.

Variant A produces K+1 independent UltraHonk proofs (K sub-proofs of
circuits/composite_plain_sort_segment + 1 glue proof of circuits/composite_plain_sort_glue),
bound by verifier-side cross-checks in pipeline/verify_composite_plain_sort.py.

Test categories (filled incrementally):
  - Valid baseline (must pass: nargo execute K+1 times, bb prove K+1 times,
    verify_composite_plain_sort.py accepts).
...
Usage:
    python tests/correctness/test_composite_plain_sort.py
...
WORK_DIR      = Path("/tmp/test_composite_plain_sort")
...
def build_hier_tomls(n, k, flat_matrix, cycle, threshold, cost, out_dir):
    """
    Run merkle_builder --composite-plain-sort K --out-dir.  Writes K+1 Prover.tomls.
    ...
    r = subprocess.run(
        [str(BUILDER_BIN), "--composite-plain-sort", str(k), "--out-dir", str(out_dir)],
...
def run_verify_hier(proof_dir, n, k):
    """Run verify_composite_plain_sort.py; return (rc, combined_output)."""
    r = subprocess.run(
...
def assert_cost_binding_rejected():
    """
    Negative test 3 (inherited from monolithic_committed): sub G5 cost binding.
...
def assert_boundary_merkle_rejected():
    """
    Negative test 4 (inherited from monolithic_committed): glue G3 boundary Merkle.
...
def main():
    ...
    print("\n--- Negative test 3: cost binding (inherited from monolithic_committed) ---")
    results.append(assert_cost_binding_rejected())

    print("\n--- Negative test 4: boundary Merkle (inherited from monolithic_committed) ---")
