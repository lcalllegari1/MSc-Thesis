"""
tests/correctness/test_composite_recursive.py

Soundness tests for the RECURSION variant (circuits/composite_recursive): the outer circuit
recursively verifies K composite_plain_product_segment (A++) proofs in-circuit and re-runs
...
KEY DIFFERENCE from test_composite_plain_product.py.  In the hierarchical family the
binding lives in pipeline/verify_composite_plain_product.py (external cross-checks), so the
...
Usage:
    python tests/correctness/test_composite_recursive.py
...
VERIFY_RECURSION = PROJECT_ROOT / "pipeline" / "verify_composite_recursive.py"
