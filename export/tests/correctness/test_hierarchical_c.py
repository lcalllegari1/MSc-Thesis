"""
tests/correctness/test_hierarchical_c.py
...
Mirrors test_hierarchical_a.py; adapts the negative tests to the committed public
...
verifier-side cross-checks in pipeline/verify_hier_c.py (same root across K+1, and
...
Test list:
  ...
  6. cross_check (NEW)        -- mix sub-proofs from cycle A with glue from cycle B;
                                 verify_hier_c rejects (C_i mismatch).
...
VERIFY_HIER    = PROJECT_ROOT / "pipeline" / "verify_hier_c.py"
BUILDER_FLAG   = "--hierarchical-c"

WORK_DIR       = Path("/tmp/test_hier_c")
...
def assert_baseline_passes():
    ...
    if rc != 0:
        print(f"  [FAIL] VALID baseline: verify_hier_c rejected:\n{out.strip()}"); return False
...
def assert_cross_check_rejected():
    ...
    verify accepts; verify_hier_c rejects because glue.C_is[i] != sub_i.C_i.
    """
    n, k = 8, 2
    flat = [0] * (n * n)
    edges = [
    ...
    if rc == 0:
        print("  [FAIL] INVALID cross-check: verify_hier_c accepted mixed proofs (BUG)")
    ...
    print("  [OK  ] INVALID cross-check: verify_hier_c rejected mixed-cycle proof set")
    return True
