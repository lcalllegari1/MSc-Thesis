"""
tests/correctness/test_hierarchical_fs.py
...
Mirrors test_hierarchical_a.py; replaces A-specific cases with A++ ones.

A++ produces K+1 independent UltraHonk proofs (K sub-proofs of
circuits/hierarchical_segment_fs + 1 glue proof of hierarchical_glue_fs), bound by
verifier-side cross-checks in pipeline/verify_hier_fs.py.
...
Test list (HIER_FS_IMPL.md sec. 8.1):
  ...
  4. cross_check_c (NEW)    -- mix sub-proofs from cycle A with glue from cycle B
                               (same matrix); verify_hier_fs rejects on c (and X).
...
WORK_DIR      = Path("/tmp/test_hier_fs")
...
def build_hier_fs_tomls(n, k, flat_matrix, cycle, threshold, cost, out_dir):
    """Run merkle_builder --hierarchical-fs K --out-dir.  Writes K+1 Prover.tomls."""
...
def run_verify_hier_fs(proof_dir, n, k):
    r = subprocess.run(
...
def assert_baseline_passes():
    """Build proofs for the reference instance; verify_hier_fs must accept."""
    ...
    build_hier_fs_tomls(inst["n"], inst["k"], inst["flat_matrix"], inst["cycle"],
    ...
    rc, out = run_verify_hier_fs(proof_dir, inst["n"], inst["k"])
    if rc != 0:
        print(f"  [FAIL] VALID baseline: verify_hier_fs rejected:\n{out.strip()}")
        return False
...
def assert_partition_overlap_rejected():
    ...
    build_hier_fs_tomls(n, k, flat, cheat_cycle, threshold=999,
...
def assert_cross_check_c_rejected():
    ...
    accepts; verify_hier_fs rejects because sub_i.c (=c_a) != glue.c (=c_b).
    ...
    build_hier_fs_tomls(n, k, flat, cycle_a, threshold=200, cost=92, out_dir=set_a_tomls)
    for i in range(k):
    ...
    build_hier_fs_tomls(n, k, flat, cycle_b, threshold=200, cost=cost_b, out_dir=set_b_tomls)
    ...
    rc, out = run_verify_hier_fs(mixed, n, k)
    if rc == 0:
        print("  [FAIL] INVALID cross-check c: verify_hier_fs accepted mixed-cycle proofs (BUG)")
        print(out)
        return False
    ...
    if "c mismatch" not in out:
        ...
        print(out)
    print("  [OK  ] INVALID cross-check c: verify_hier_fs rejected mixed-cycle proof set (c binding)")
    return True
...
def assert_baseline_witness_passes_at(n, k):
    ...
    build_hier_fs_tomls(n, k, flat, cycle, threshold=math.ceil(cost * 1.1), cost=cost,
                        out_dir=hier_dir)
    for i in range(k):
        rc, err = run_sub_witness(i, hier_dir)
        if rc != 0:
