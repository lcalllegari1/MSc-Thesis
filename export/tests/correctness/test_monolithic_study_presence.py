"""
tests/correctness/test_monolithic_study_presence.py

Soundness tests for circuits/monolithic_study_presence.
...
The circuit under test enforces four constraint groups:
  ...
  GROUP 2  presence check: seen[cycle[i]] == false before marking it true
           (unlike monolithic_study_sort and monolithic_study_invperm, GROUP 1 is NOT
...
Usage:
    python tests/correctness/test_monolithic_study_presence.py
...
CIRCUIT_DIR  = PROJECT_ROOT / "circuits" / "monolithic_study_presence"
...
def make_inputs(n, cycle, matrix=None, threshold_slack=0.1):
    """
    Build a complete inputs dict for monolithic_study_presence.

    No extra witness fields -- the circuit takes only cycle, cost_matrix,
    and threshold (unlike monolithic_study_invperm which also needs inv_perm).
...
def main():
    print("Soundness tests: circuits/monolithic_study_presence")
    print("=" * 52)
