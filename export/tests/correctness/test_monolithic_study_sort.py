"""
tests/correctness/test_monolithic_study_sort.py

Soundness tests for circuits/monolithic_study_sort.
...
The circuit under test enforces three constraint groups:
  GROUP 2  sort-based permutation:  sorted(cycle) == [0, 1, ..., N-1]
...
Note: there is no separate GROUP 1 in this circuit. Out-of-range node indices
and duplicate nodes are both caught by GROUP 2 (the sorted cycle would not
equal [0,...,N-1] in either case).

A sound circuit must catch *every* case below. If a test marked INVALID
passes (exit code 0), the circuit has a soundness hole.

Usage:
    python tests/correctness/test_monolithic_study_sort.py
...
CIRCUIT_DIR  = PROJECT_ROOT / "circuits" / "monolithic_study_sort"
...
def make_inputs(n, cycle, matrix=None, threshold_slack=0.1):
    ...
    fall back to threshold=1 — the circuit will reject at GROUP 2 regardless.
    """
    if matrix is None:
    ...
    try:
        ...
        # Out-of-range node index: dummy values are fine — the circuit rejects at GROUP 2.
        cost = 0
        threshold = 1
    return {
        "n": n,
...
def run_tests_n5():
    ...
    # ── INVALID cases — GROUP 2: out-of-range indices ────────────────────────
    # In monolithic_study_sort, GROUP 1 (range check) is subsumed by GROUP 2.
    # An out-of-range index causes sorted(cycle) != [0,...,N-1].

    # One index equals N (out of bounds by 1)
    bad = [0, 1, 2, 3, n]
    results.append(assert_invalid(make_inputs(n, bad), "cycle[4] = N (out of range by 1)"))

    # One index far out of range
    bad = [0, 1, 2, 3, 99]
    results.append(assert_invalid(make_inputs(n, bad), "cycle[4] = 99 (far out of range)"))

    # ── INVALID cases — GROUP 2: duplicate nodes ─────────────────────────────
    ...
    flat[1] = flat[1] * 1000 + 1000   # edge 0->1 is at flat index 0*5+1 = 1
...
def main():
    print("Soundness tests: circuits/monolithic_study_sort")
    print("=" * 52)
