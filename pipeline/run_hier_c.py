"""
pipeline/run_hier_c.py -- Variant committed-A hierarchical TSP benchmark harness.

Structurally identical to pipeline/run_hier.py (Variant A); the differences are
mechanical:
  * Circuits: hierarchical_segment_c / hierarchical_glue_c (Variant committed-A:
  ...
  * Builder:  merkle_builder --hierarchical-c K  (folds each segment + a uniform
              blinding r into C_i; the glue recomputes and binds them).
  * Verifier: pipeline/verify_hier_c.py (cross-checks the shared root and that
              glue.C_is[i] == sub_i.C_i; no Fiat-Shamir, no per-field bookkeeping).
  * Shadows:  /tmp/hier_c_shadows (does not collide with A's /tmp/hier_a_shadows).
  * CSV variant column: "hier_c".

The CSV schema is identical to run_hier.py's (K+1 rows per cell), so
pipeline/aggregate_hier.py consumes results/hier_c.csv with no changes and emits
hier_c_k{K} variant rows for plot.py.
...
For each (N, K) the script:
  ...
  3. For each run: generates an instance, solves a cycle, builds the K+1
     Prover.tomls via merkle_builder --hierarchical-c, then launches K+1
     parallel `nargo execute && bb prove` workers.
  4. Runs verify_hier_c.py to sanity-check the cross-checks.
...
Usage:
    python pipeline/run_hier_c.py \\
        --ns 48 96 192 480 \\
        --ks 2 4 8 \\
        --runs 3 \\
        --out results/hier_c.csv
"""
...
SUB_DIR      = PROJECT_ROOT / "circuits" / "hierarchical_segment_c"
GLUE_DIR     = PROJECT_ROOT / "circuits" / "hierarchical_glue_c"
SUB_NAME     = "hierarchical_segment_c"
GLUE_NAME    = "hierarchical_glue_c"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
VERIFY_HIER_C = PROJECT_ROOT / "pipeline" / "verify_hier_c.py"

# Per-N-K sub-circuit shadow dirs, hosted under /tmp/ (see run_hier.py rationale).
# Distinct from A's /tmp/hier_a_shadows so both harnesses can run concurrently.
SHADOW_ROOT  = Path("/tmp/hier_c_shadows")
...
FIELDNAMES = [
    ...
    "verify_hier_s", # wall time for pipeline/verify_hier_c.py (same value all K+1 rows)
    "xchecks_ok",    # 1 if verify_hier_c accepted, else 0
]
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --hierarchical-c."""
...
def build_hier_tomls(payload_dict, k, out_dir):
    """Invoke merkle_builder --hierarchical-c K --out-dir <dir>."""
    if out_dir.exists():
        shutil.rmtree(out_dir)
    payload = json.dumps(payload_dict)
    r = subprocess.run(
        [str(BUILDER_BIN), "--hierarchical-c", str(k), "--out-dir", str(out_dir)],
...
def parallel_prove_all(k, hier_dir, scratch):
    """Launch K+1 parallel workers and collect metrics (see run_hier.py)."""
    results = []
    procs   = []
    meta_paths = []
...
def run_verify_hier_c(scratch, n, k):
    """Run pipeline/verify_hier_c.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
        ...
        for n in ns:
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    cell_dir  = Path(f"/tmp/run_hier_c/n{n}_k{k}/run{run_idx}")
                    if cell_dir.exists():
                    ...
                    vh_rc, vh_s = run_verify_hier_c(scratch, n, k)
                    ...
                    print(
                        ...
                        f"verify_hier_c={'OK' if xcheck_ok else 'FAIL'}({vh_s:.2f}s)"
                    )
...
def main():
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/hier_c.csv")
