"""
pipeline/run_hier_cfs.py -- Variant committed-A++ hierarchical TSP benchmark harness.

Structurally identical to pipeline/run_hier.py (Variant A); the differences are
mechanical (HIER_FS_IMPL.md sec. 6):
  * Circuits: hierarchical_segment_cfs / hierarchical_glue_cfs.
  * Builder:  merkle_builder --hierarchical-cfs K  (adds the FS chain, the
              per-segment grand products P_i, and the chain anchors).
  * Verifier: pipeline/verify_hier_cfs.py (cross-checks root/c/X and the
              per-segment P_i / h_in / h_out, no field arithmetic).
  * Shadows:  /tmp/hier_cfs_shadows (does not collide with A's /tmp/hier_a_shadows).
  * CSV variant column: "hier_cfs".

The CSV schema is identical to run_hier.py's (K+1 rows per cell), so
pipeline/aggregate_hier.py consumes results/hier_cfs.csv with no changes and emits
hier_cfs_k{K} variant rows for plot.py.
...
For each (N, K) the script:
  ...
  3. For each run: generates an instance, solves a cycle, builds the K+1
     Prover.tomls via merkle_builder --hierarchical-cfs, then launches K+1
     parallel `nargo execute && bb prove` workers.
  4. Runs verify_hier_cfs.py to sanity-check the cross-checks.
...
Usage:
    python pipeline/run_hier_cfs.py \\
        --ns 48 96 192 480 \\
        --ks 2 4 8 \\
        --runs 3 \\
        --out results/hier_cfs.csv
"""
...
SUB_DIR      = PROJECT_ROOT / "circuits" / "hierarchical_segment_cfs"
GLUE_DIR     = PROJECT_ROOT / "circuits" / "hierarchical_glue_cfs"
SUB_NAME     = "hierarchical_segment_cfs"
GLUE_NAME    = "hierarchical_glue_cfs"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
VERIFY_HIER_CFS = PROJECT_ROOT / "pipeline" / "verify_hier_cfs.py"
...
SHADOW_ROOT  = Path("/tmp/hier_cfs_shadows")
...
FIELDNAMES = [
    ...
    "verify_hier_s", # wall time for pipeline/verify_hier_cfs.py (same value all K+1 rows)
    "xchecks_ok",    # 1 if verify_hier_cfs accepted, else 0
]
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --hierarchical-cfs."""
...
def build_hier_tomls(payload_dict, k, out_dir):
    """Invoke merkle_builder --hierarchical-cfs K --out-dir <dir>."""
    if out_dir.exists():
        shutil.rmtree(out_dir)
    payload = json.dumps(payload_dict)
    r = subprocess.run(
        [str(BUILDER_BIN), "--hierarchical-cfs", str(k), "--out-dir", str(out_dir)],
...
def run_verify_hier_cfs(scratch, n, k):
    """Run pipeline/verify_hier_cfs.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
        ...
        for n in ns:
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    cell_dir  = Path(f"/tmp/run_hier_cfs/n{n}_k{k}/run{run_idx}")
                    if cell_dir.exists():
                    ...
                    vh_rc, vh_s = run_verify_hier_cfs(scratch, n, k)
                    ...
                    print(
                        ...
                        f"verify_hier_cfs={'OK' if xcheck_ok else 'FAIL'}({vh_s:.2f}s)"
                    )
...
def main():
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/hier_cfs.csv")
