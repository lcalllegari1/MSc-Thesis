"""
pipeline/run_hier_fs.py -- Variant A++ hierarchical TSP benchmark harness.

Structurally identical to pipeline/run_hier.py (Variant A); the differences are
mechanical (HIER_FS_IMPL.md sec. 6):
  * Circuits: hierarchical_segment_fs / hierarchical_glue_fs.
  * Builder:  merkle_builder --hierarchical-fs K  (adds the FS chain, the
              per-segment grand products P_i, and the chain anchors).
  * Verifier: pipeline/verify_hier_fs.py (cross-checks root/c/X and the
              per-segment P_i / h_in / h_out, no field arithmetic).
  * Shadows:  /tmp/hier_fs_shadows (does not collide with A's /tmp/hier_a_shadows).
  * CSV variant column: "hier_fs".

The CSV schema is identical to run_hier.py's (K+1 rows per cell), so
pipeline/aggregate_hier.py consumes results/hier_fs.csv with no changes and emits
hier_fs_k{K} variant rows for plot.py.
...
For each (N, K) the script:
  ...
  3. For each run: generates an instance, solves a cycle, builds the K+1
     Prover.tomls via merkle_builder --hierarchical-fs, then launches K+1
     parallel `nargo execute && bb prove` workers.
  4. Runs verify_hier_fs.py to sanity-check the cross-checks.
...
GLUE_DIR     = PROJECT_ROOT / "circuits" / "hierarchical_glue_fs"
SUB_NAME     = "hierarchical_segment_fs"
GLUE_NAME    = "hierarchical_glue_fs"
BUILDER_BIN  = PROJECT_ROOT / "pipeline" / "merkle_builder" / "target" / "release" / "merkle_builder"
VERIFY_HIER_FS = PROJECT_ROOT / "pipeline" / "verify_hier_fs.py"
...
SHADOW_ROOT  = Path("/tmp/hier_fs_shadows")
...
FIELDNAMES = [
    ...
    "verify_hier_s", # wall time for pipeline/verify_hier_fs.py (same value all K+1 rows)
    "xchecks_ok",    # 1 if verify_hier_fs accepted, else 0
]
...
def write_inputs_json(n, k, instance, cycle, multiplier=1.1):
    """Build the JSON payload for merkle_builder --hierarchical-fs."""
...
def run_verify_hier_fs(scratch, n, k):
    """Run pipeline/verify_hier_fs.py against scratch/.  Returns (rc, elapsed)."""
    cmd = [
        ...
        for n in ns:
            for k in ks:
                ...
                for run_idx in range(1, runs + 1):
                    ...
                    vh_rc, vh_s = run_verify_hier_fs(scratch, n, k)
                    ...
                    print(
                        ...
                        f"verify_hier_fs={'OK' if xcheck_ok else 'FAIL'}({vh_s:.2f}s)"
                    )
...
def main():
    ...
    ap.add_argument("--out", required=True,
                    help="Output CSV path, e.g. results/hier_fs.csv")
