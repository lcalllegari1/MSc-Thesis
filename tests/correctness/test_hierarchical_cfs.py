"""
tests/correctness/test_hierarchical_cfs.py
...
Mirrors test_hierarchical_fs.py; adapts the negative tests to the committed
...
circuits/hierarchical_segment_cfs + 1 glue proof of hierarchical_glue_cfs), bound
by verifier-side cross-checks in pipeline/verify_hier_cfs.py (same root + X across
...
Test list:
  ...
  6. cross_check (NEW)        -- mix sub-proofs from cycle A with glue from cycle B;
                                 verify_hier_cfs rejects (X and C_i mismatch).
...
VERIFY_HIER    = PROJECT_ROOT / "pipeline" / "verify_hier_cfs.py"
BUILDER_FLAG   = "--hierarchical-cfs"

WORK_DIR       = Path("/tmp/test_hier_cfs")
...
def assert_baseline_passes():
    ...
    if rc != 0:
        print(f"  [FAIL] VALID baseline: verify_hier_cfs rejected:\n{out.strip()}"); return False
...
def assert_cross_check_rejected():
    ...
    verify_hier_cfs rejects because sub_i.X != glue.X (and C_i mismatch).
    """
    n, k = 8, 2
    flat = [0] * (n * n)
    edges = [
    ...
    if rc == 0:
        print("  [FAIL] INVALID cross-check: verify_hier_cfs accepted mixed proofs (BUG)")
    ...
    print("  [OK  ] INVALID cross-check: verify_hier_cfs rejected mixed-cycle proof set")
    return True
