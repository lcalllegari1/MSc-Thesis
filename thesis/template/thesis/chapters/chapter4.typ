...
The second choice is how to enforce Group 1, that the cycle is a permutation of the nodes. A permutation, taken whole, does not suggest any one check, so it helps to see it as two requirements held together: that no node is visited twice, and that no node is missed. Each node, in other words, is visited _exactly_ once, which is _at most_ once and _at least_ once at the same time. We take the first half first, because it is the one with an obvious direct route. Suppose you wanted to check, in any ordinary program, that a list of $n$ numbers has no repeats: you would compare every entry against every other and make sure no two are equal. That pairwise check is the first thing we write, and it secures the _at most once_ half exactly: no node is visited twice. The _at least once_ half it cannot reach on its own, and the reason is worth seeing. The pairwise check never asks whether the entries are real nodes, so a prover could hand over $n$ numbers that are all distinct yet run past the end of the graph: no repeats, and no tour either. To rule this out we add a _range check_, which forces every entry to be a valid node index. Now the two halves meet, and the second falls out of the first for free: $n$ entries that are all distinct and all in range cannot leave any node out, because by the pigeonhole principle $n$ distinct values drawn from the $n$ nodes must be all of them, each used once. Distinctness and range together give back the full permutation. The check is correct, but it is quadratic, as the gate count will soon remind us. Cheaper checks exist, and a stronger one will later enforce the permutation directly, with no separate range check. For now, the brute-force version is the honest starting point.
...
The first of those two questions is the subject of this section. It has more than one answer: five in total#footnote[These five are the mechanisms we build and measure, not an exhaustive catalogue, and two omissions are worth naming for completeness. The grand product of @sub:grand-product has a more efficient modern relative, the _logarithmic-derivative_ argument (LogUp), which settles the same multiset equality by comparing sums of inverses $sum 1\/(X + a_i)$ in place of products @habock2022logup; it occupies the very cell of our analysis the grand product does---probabilistic and free of dynamic memory---and would sharpen the constants without disturbing any structural claim. In the opposite corner sit _permutation networks_ (Beneš, Waksman), which route the cycle onto the identity through $O(n log n)$ conditional-swap switches whose settings are the witness @benes1965networks @waksman1968permutation; these are exact, free of dynamic memory, _and_ sub-quadratic all at once---a combination our five reach only two at a time, the pairwise check being exact and static but quadratic, the grand product static and scalable but probabilistic---yet under the decomposition of @chap:hierarchical such a network must publish its segment's node set exactly as the sort does, so it inherits the sort's disclosure at a higher gate cost and stakes out no position the survivors do not already hold. Lookup arguments---plookup @gabizon2020 and its successors---prove a permutation as the special case of multiset equality against a fixed table, and are the batched form of the same machinery.], of which the pairwise check of @sec:proof-anatomy is the first and the worst. That check is correct but quadratic, comparing every position against every other, $n(n-1) \/ 2$ comparisons in all. What we want is a check that scales linearly with $n$. The four candidates ahead all reach that goal, by different routes and at different costs.
